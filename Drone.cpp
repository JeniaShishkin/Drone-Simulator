...
Drone::Drone(WorldGrid grid, Compass compass, Position position, unsigned int energy, Sensors sensors) : 
		m_grid(grid), m_compass(compass), m_position(position), m_energy(energy), m_sensors(sensors), m_ctx{m_grid, m_position, m_compass}
{
	...
	for (auto& s : sensors) {
	 ...
	 m_commands.emplace("fwd", std::make_unique<FixedConsumption<CmdForward, 10>>(&m_grid, &m_position, &m_compass));
	 ...
	 m_commands.emplace("map", std::make_unique<FixedConsumption<CmdMap, 10>>(m_grid));
